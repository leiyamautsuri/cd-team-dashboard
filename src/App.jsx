...
export default function Dashboard() {
  ...
  useEffect(() => {
    async function load() {
      try {
        const result = (() => { try { const r = localStorage.getItem("cd-dashboard-data"); return r ? {value: r} : null; } catch(e) { return null; } })();
  ...
  const save = useCallback(async (newData) => {
    setSaving(true);
    try {
      localStorage.setItem("cd-dashboard-data", JSON.stringify(newData));
      setSaveMsg("✓ 保存しました");
